...
const SNAKE_CSS = `
    ...
    .bf-wing-l {
        ...
        animation: bfWingFlap 0.30s ease-in-out infinite;
    }
    .bf-wing-r {
        transform-box: fill-box;
        transform-origin: 0% 50%;
        animation: bfWingFlap 0.30s 0.15s ease-in-out infinite;
    }
    .sna-wood {
        background:
            repeating-linear-gradient(
                ...
                rgba(0,0,0,0.055) 5px, rgba(0,0,0,0.055) 6px,
...
function SnakeSingle({ onBack, settings, wordDatabase, qualifyingBook, onSaveScore }) {
    ...
    useEffect(() => {
        ...
        let damageFlashStart = 0; // 傷害紅色閃爍計時

        const GRID_W = 24, GRID_H = 14, TILE_SIZE = 40;
        canvas.width  = GRID_W * TILE_SIZE;  // 960
        canvas.height = GRID_H * TILE_SIZE;  // 560
            ...
            for (let gx = 0; gx < GRID_W; gx++) {
                ...
                if (!isBorder) {
                    ...
                    for (let b = 0; b < bushCount; b++) {
                        ...
                        bgCtx.beginPath();
                        bgCtx.arc(
                            tx + TILE_SIZE * (0.15 + pr(gx * 31 + gy * 13 + b * 7) * 0.7),
                            ty + TILE_SIZE * (0.15 + pr(gx * 23 + gy * 17 + b * 5) * 0.7),
                            TILE_SIZE * (0.22 + pr(gx * 19 + gy * 11 + b * 3) * 0.2),
                            0, Math.PI * 2
                        );
                        bgCtx.fill();
                    }
                    ...
                    bgCtx.beginPath();
                    bgCtx.arc(
                        tx + TILE_SIZE * (0.22 + pr(gx * 7 + gy * 29) * 0.25),
                        ty + TILE_SIZE * (0.16 + pr(gx * 11 + gy * 23) * 0.2),
                        TILE_SIZE * 0.07, 0, Math.PI * 2
                    );
                    bgCtx.fill();
                }
            }
            ...
            for (let i = 0; i < targetWordStr.length; i++) {
                ...
                while (!isValid) {
                    ...
                    isValid = !snake.some(s => s.x === newX && s.y === newY)
                           && !mapLetters.some(l => l.x === newX && l.y === newY);
                }
        ...
        const endGame = () => {
            ...
            const hearts    = Math.max(0, engineHearts);
            const totalScore = engineScore + hearts * 10;
        ...
        const drawGame = () => {
            ...
            mapLetters.forEach(letter => {
                ...
                const isEasyTarget = isEasy && isTarget;
                ...
                ctx.beginPath();
                ctx.arc(lx, ly, TILE_SIZE * 0.42, 0, Math.PI * 2);
                ctx.fill();
                ...
                ctx.lineWidth = 1.8;
                ctx.beginPath();
                ctx.moveTo(lx, ly - TILE_SIZE * 0.42);
                ctx.lineTo(lx + 3, ly - TILE_SIZE * 0.54);
                ctx.stroke();

                // 高光
                ctx.fillStyle = 'rgba(255,255,255,0.3)';
                ctx.beginPath();
                ctx.ellipse(lx - TILE_SIZE * 0.1, ly - TILE_SIZE * 0.12, TILE_SIZE * 0.13, TILE_SIZE * 0.09, -0.5, 0, Math.PI * 2);
                ctx.fill();
            ...
            snake.forEach((segment, index) => {
                ...
                if (isInvincible) {
                    ctx.globalAlpha = Math.floor(Date.now() / 200) % 2 === 0 ? 0.28 : 0.78;
                }

                if (index === 0) {
                    // 蛇頭
                    ctx.fillStyle = isInvincible ? '#38bdf8' : '#15803d';
                    snakeRoundRect(ctx, sx + 2, sy + 2, TILE_SIZE - 4, TILE_SIZE - 4, 9);
                    ctx.fill();

                    // 頭部紋路
                    if (!isInvincible) {
                        ctx.fillStyle = '#166534';
                        snakeRoundRect(ctx, sx + 6, sy + 6, TILE_SIZE - 12, TILE_SIZE - 12, 5);
                        ctx.fill();
                    }

                    // 眼睛（依方向定位）
                    const eyePos = {
                        RIGHT: [{ ex: 0.72, ey: 0.28 }, { ex: 0.72, ey: 0.72 }],
                        LEFT:  [{ ex: 0.28, ey: 0.28 }, { ex: 0.28, ey: 0.72 }],
                        UP:    [{ ex: 0.28, ey: 0.28 }, { ex: 0.72, ey: 0.28 }],
                        DOWN:  [{ ex: 0.28, ey: 0.72 }, { ex: 0.72, ey: 0.72 }],
                    ...
                    eyePos.forEach(e => {
                        ...
                        ctx.fillStyle = 'white';
                        ctx.beginPath();
                        ctx.arc(sx + e.ex * TILE_SIZE, sy + e.ey * TILE_SIZE, 5.5, 0, Math.PI * 2);
                        ctx.fill();
                        ctx.fillStyle = '#111';
                        const pupilOffset = { RIGHT:[1.5,0], LEFT:[-1.5,0], UP:[0,-1.5], DOWN:[0,1.5] }[currentDir] || [0,0];
                        ctx.beginPath();
                        ctx.arc(sx + e.ex * TILE_SIZE + pupilOffset[0], sy + e.ey * TILE_SIZE + pupilOffset[1], 2.8, 0, Math.PI * 2);
                        ctx.fill();
                    });
                    ctx.globalAlpha = 1;
                    ...
                    if (!isInvincible) {
                        const tongueBase = {
                            RIGHT: { x: sx + TILE_SIZE,     y: sy + TILE_SIZE / 2, dx: 9,  dy: 0  },
                            LEFT:  { x: sx,                 y: sy + TILE_SIZE / 2, dx: -9, dy: 0  },
                            UP:    { x: sx + TILE_SIZE / 2, y: sy,                 dx: 0,  dy: -9 },
                            DOWN:  { x: sx + TILE_SIZE / 2, y: sy + TILE_SIZE,     dx: 0,  dy: 9  },
                        }[currentDir] || { x: sx + TILE_SIZE, y: sy + TILE_SIZE/2, dx: 9, dy: 0 };

                        ctx.strokeStyle = '#dc2626';
                        ctx.lineWidth = 2;
                        const { x: tbx, y: tby, dx, dy } = tongueBase;
                        ctx.beginPath(); ctx.moveTo(tbx, tby); ctx.lineTo(tbx + dx, tby + dy); ctx.stroke();
                        const perpX = dy !== 0 ? 4 : 0, perpY = dx !== 0 ? 4 : 0;
                        ctx.beginPath(); ctx.moveTo(tbx + dx, tby + dy); ctx.lineTo(tbx + dx + perpX, tby + dy + perpY); ctx.stroke();
                        ctx.beginPath(); ctx.moveTo(tbx + dx, tby + dy); ctx.lineTo(tbx + dx - perpX, tby + dy - perpY); ctx.stroke();
                    }
                    ...
                    ctx.fillStyle = isInvincible
                        ? (even ? '#bae6fd' : '#93c5fd')
                        : (even ? '#16a34a' : '#166534');
                    snakeRoundRect(ctx, sx + 3, sy + 3, TILE_SIZE - 6, TILE_SIZE - 6, 6);
                    ctx.fill();
    ...
    if (!gameStarted) return (
        <div style={{ width:'100%', height:'100svh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:24, background:'radial-gradient(ellipse at 50% 60%, #166534 0%, #052e16 55%, #021a0e 100%)', position:'relative', overflow:'hidden' }}>
            ...
            {/* 背景花朵裝飾 */}
            {[...Array(14)].map((_, i) => {
                const fc = ['#fb7185','#fbbf24','#a78bfa','#60a5fa','#f9a8d4'];
                const lx = 5 + (i * 7.1) % 88, ly = 5 + (i * 13.3) % 88;
                const fsz = 18 + (i % 3) * 6;
                return (
                    <div key={i} style={{ position:'absolute', left:`${lx}%`, top:`${ly}%`, opacity: 0.3 + (i % 4) * 0.08, pointerEvents:'none' }}>
                        <svg viewBox="0 0 30 30" width={fsz} height={fsz}>
                            {[0,1,2,3,4].map(p => {
                                const a = (p/5)*Math.PI*2 - Math.PI/2;
                                return <ellipse key={p} cx={15+Math.cos(a)*9} cy={15+Math.sin(a)*9} rx="5.5" ry="3.2" transform={`rotate(${p*72 - 90},${15+Math.cos(a)*9},${15+Math.sin(a)*9})`} fill={fc[p]}/>;
                            })}
                            <circle cx="15" cy="15" r="4" fill="#fef08a"/>
                        </svg>
                    </div>
                );
            })}
            {/* 主卡片（木牌風格）*/}
            <div className="sna-wood" style={{ borderRadius:24, padding:40, maxWidth:440, width:'100%', textAlign:'center' }}>
                <div style={{ fontSize:58, marginBottom:14, lineHeight:1 }}>🐍</div>
                <h1 style={{ fontSize:28, fontWeight:900, color:'#fef3c7', marginBottom:8, letterSpacing:2, textShadow:'0 2px 8px rgba(0,0,0,0.55)' }}>貪食蛇大冒險</h1>
                <p style={{ color:'#fde68a', marginBottom:28, fontWeight:600, opacity:0.85 }}>選擇挑戰難度</p>
                <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
                    <button onClick={() => { playVoice(""); setGameMode('easy'); setGameStarted(true); }}
                        className="sna-wood-btn" style={{ padding:'18px 24px', borderRadius:14, fontWeight:900, fontSize:18 }}>
                        🌟 簡易模式
                        <div style={{ fontSize:13, fontWeight:500, opacity:0.78, marginTop:5 }}>下一個字母會發光提示</div>
                    </button>
                    <button onClick={() => { playVoice(""); setGameMode('normal'); setGameStarted(true); }}
                        className="sna-wood-btn" style={{ padding:'18px 24px', borderRadius:14, fontWeight:900, fontSize:18, background:'repeating-linear-gradient(88deg,transparent 0px,transparent 5px,rgba(0,0,0,0.055) 5px,rgba(0,0,0,0.055) 6px,transparent 6px,transparent 13px), linear-gradient(160deg,#92400e,#78350f,#451a03)' }}>
                        🔥 一般模式
                        <div style={{ fontSize:13, fontWeight:500, opacity:0.78, marginTop:5 }}>無提示，自動存入榮譽榜</div>
                    </button>
                </div>
                <button onClick={onBack} style={{ marginTop:22, color:'#fde68a', background:'none', border:'none', cursor:'pointer', fontWeight:700, fontSize:14, opacity:0.75 }}>
                    ← 回大廳
                </button>
            </div>
        </div>
        ...
        <div style={{ width:'100%', height:'100svh', display:'flex', flexDirection:'column', position:'relative', touchAction:'none', overscrollBehavior:'none', overflow:'hidden', background:'#052e16' }}>
            ...
            <div style={{ position:'absolute', top:0, width:'100%', display:'flex', justifyContent:'space-between', alignItems:'center', padding:'10px 16px', zIndex:10, background:'linear-gradient(90deg,#3a1a00,#713f12 50%,#3a1a00)', borderBottom:'2px solid #92400e', boxShadow:'0 2px 16px rgba(0,0,0,0.55)', boxSizing:'border-box' }}>
                <div style={{ display:'flex', gap:10, alignItems:'center' }}>
                    <button onClick={onBack} className="sna-wood-btn" style={{ padding:'8px 14px', borderRadius:12, fontSize:14 }}>← 放棄</button>
                ...
                <div className="sna-wood" style={{ borderRadius:10, padding:'7px 16px', fontWeight:800, color:'#fef3c7', fontSize:16 }}>
                    🌿 {scoreUI}
                </div>
            </div>
                ...
                <div style={{ position:'absolute', inset:0, zIndex:50, background:'rgba(5,46,22,0.96)', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:24, animation:'snkFadeIn .5s ease-out' }}>
                    ...
                    <div className="sna-wood" style={{ borderRadius:20, padding:24, marginBottom:20, width:'100%', maxWidth:340 }}>
                        <div style={{ display:'flex', justifyContent:'space-between', marginBottom:10, fontWeight:700, color:'#fde68a' }}>
                            <span>遊戲得分</span><span>{scoreUI}</span>
                        </div>
                        <div style={{ display:'flex', justifyContent:'space-between', paddingBottom:12, marginBottom:12, borderBottom:'1px solid rgba(255,255,255,0.15)', fontWeight:700, color:'#fda4af' }}>
                            <span>愛心紅利 ({heartsUI}×10)</span><span>+{heartsUI * 10}</span>
                        </div>
                        <div style={{ display:'flex', justifyContent:'space-between', fontWeight:900, fontSize:22, color:'#86efac' }}>
                            <span>總結算</span><span>{finalScoreUI}</span>
                        </div>
                    </div>
                    {gameMode === 'normal' && qualifyingBook ? (
                        <p style={{ color:'#86efac', fontWeight:700, marginBottom:20, padding:'12px 20px', borderRadius:12, background:'rgba(134,239,172,.1)', border:'1px solid rgba(134,239,172,.3)', fontSize:14 }}>
                            ☁️ 個人最佳成績已自動存檔
                        </p>
                    ) : (
                        <p style={{ color:'#a16207', fontSize:13, marginBottom:20, fontWeight:600 }}>（簡易/練習模式，不列入排行榜）</p>
                    )}
                    <button onClick={onBack} className="sna-wood-btn" style={{ padding:'14px 40px', borderRadius:16, fontWeight:900, fontSize:18 }}>
                        🌿 回主畫面
                    </button>
                </div>
            )}
                ...
                <div style={{ position:'absolute', left:`${bfPos.x}%`, top:`${bfPos.y}%`, transform:`translate(-50%,-50%) scaleX(${bfFlip ? -1 : 1})`, pointerEvents:'none', zIndex:5, animation:'bfFloat 1.9s ease-in-out infinite' }}>
                    <svg viewBox="0 0 62 44" width="46" height="34">
                        <g className="bf-wing-l">
                            <ellipse cx="22" cy="17" rx="20" ry="14" fill="#fb923c" opacity="0.76"/>
                            <ellipse cx="17" cy="31" rx="14" ry="9"  fill="#fed7aa" opacity="0.70"/>
                        </g>
                        <g className="bf-wing-r">
                            <ellipse cx="40" cy="17" rx="20" ry="14" fill="#fb923c" opacity="0.76"/>
                            <ellipse cx="45" cy="31" rx="14" ry="9"  fill="#fed7aa" opacity="0.70"/>
                        </g>
                        <ellipse cx="31" cy="22" rx="2.8" ry="13" fill="#1c1917"/>
                        <line x1="31" y1="9"  x2="23" y2="2"  stroke="#1c1917" strokeWidth="1.5"/>
                        <circle cx="22" cy="1.5" r="2.2" fill="#1c1917"/>
                        <line x1="31" y1="9"  x2="39" y2="2"  stroke="#1c1917" strokeWidth="1.5"/>
                        <circle cx="40" cy="1.5" r="2.2" fill="#1c1917"/>
                    </svg>
                </div>
            )}
            ...
            {/* ── 動物探頭（從底部灌木叢）── */}
            {animalPeek && (
                <div key={animalPeek.key}
                    style={{ position:'absolute', bottom:0, left:`${animalPeek.pos}%`, pointerEvents:'none', zIndex:6 }}>
            ...
            {/* ── 橫向提示（原版保留）── */}
            <div style={{ position:'fixed', inset:0, background:'rgba(5,46,22,0.97)', zIndex:100, display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', color:'white', padding:24, textAlign:'center' }} className="portrait:flex landscape:hidden">
                <i className="fa-solid fa-mobile-screen" style={{ fontSize:54, marginBottom:18, transform:'rotate(-90deg)', color:'#4ade80' }}></i>
                <h2 style={{ fontSize:26, fontWeight:900, marginBottom:8, color:'#86efac' }}>設備轉向提醒</h2>
                <p style={{ color:'#bbf7d0', fontSize:15 }}>請將平板或手機轉為<strong>橫向</strong><br/>以獲得最佳視野與操控體驗</p>
            </div>
        </div>
    );
}
