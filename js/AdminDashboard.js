...
function AdminDashboard({ onBack, dbRef, lang = 'zh-TW', setLang }) {
    ...
    const [targetWeek, setTargetWeek] = useState(() => {
        ...
        return getWeekSafe > 1 ? getWeekSafe - 1 : 1;
    });

    const dict = {
        'zh-TW': {
            adminLogin: '管理員登入', pwdPlaceholder: '請輸入授權密碼', login: '登入',
            wrongPwd: '密碼錯誤，存取被拒。', tabRooms: '幽靈房間管理', tabScores: '異常分數管理',
            tabSnapshot: '歷史快照封存', refresh: '重新整理', delete: '刪除',
            confirmDelete: '確定要刪除嗎？此操作無法復原。', noData: '目前沒有資料',
            roomCode: '房號', status: '狀態', player: '玩家', score: '分數', mode: '模式',
            packSnapshot: '打包並封存當週快照',
            snapshotWarning: '這將會把該週所有成績打包成 1 個 JSON 檔案，並清除原始資料以節省空間。確定執行？',
            packSuccess: '快照封存成功！原始資料已清理。'
        },
        'en': {
            adminLogin: 'Admin Login', pwdPlaceholder: 'Enter authorization code', login: 'Login',
            wrongPwd: 'Password incorrect, access denied.', tabRooms: 'Ghost Rooms', tabScores: 'Manage Scores',
            tabSnapshot: 'History Snapshots', refresh: 'Refresh', delete: 'Delete',
            confirmDelete: 'Are you sure? This cannot be undone.', noData: 'No data available',
            roomCode: 'Code', status: 'Status', player: 'Player', score: 'Score', mode: 'Mode',
    ...
    const clearGhostRooms = async () => {
        ...
        try {
            ...
            ghosts.forEach(room => batch.delete(dbRef.collection('rooms').doc(room.id)));
            await batch.commit();
            setRooms(prev => prev.filter(r => r.status === 'playing'));
            showMessage(`大掃除完成！共清除了 ${ghosts.length} 個幽靈房間。`);
        } catch (e) { showMessage('清除失敗', 'error'); }
        setLoading(false);
    };
    ...
    const createSnapshot = async () => {
        ...
        try {
            ...
            Object.keys(grouped).forEach(key => {
                // 安全排序：確保遇到沒有時間的資料不會算成 NaN 導致排行榜錯亂
        ...
        } catch (e) { showMessage('快照打包發生錯誤', 'error'); }
        setLoading(false);
    };
            ...
            <main className="flex-1 bg-slate-900 rounded-3xl border border-slate-800 p-4 sm:p-6 overflow-hidden flex flex-col">
                    ...
                    <div className="flex flex-col h-full">
                        <div className="flex flex-wrap justify-between items-end mb-6 gap-4">
                            <div>
                                ...
                                <div className="flex gap-3 text-sm font-bold">
                                    ...
                                    <span className="bg-slate-700 text-slate-400 px-3 py-1 rounded-lg">幽靈: {rooms.filter(r => r.status !== 'playing').length}</span>
                                </div>
                            </div>
                        ...
                        <div className="overflow-y-auto flex-1 custom-scrollbar bg-slate-950/50 rounded-xl border border-slate-800">
                            <table className="w-full text-left">
                                <thead className="bg-slate-800 text-slate-400 sticky top-0"><tr><th className="p-3">ID</th><th className="p-3">{t.roomCode}</th><th className="p-3">{t.status}</th><th className="p-3">操作</th></tr></thead>
                                <tbody>
                                    {rooms.length === 0 ? <tr><td colSpan="4" className="text-center p-8 text-slate-500">{t.noData}</td></tr> : rooms.map(r => (
                                        <tr key={r.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                                            ...
                                            <td className="p-3"><button onClick={() => deleteRoom(r.id)} className="px-3 py-1 bg-red-900/50 hover:bg-red-600 text-red-200 rounded font-bold">刪除</button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                {activeTab === 'scores' && (
                    <div className="flex flex-col h-full">
                        ...
                        <div className="overflow-y-auto flex-1 custom-scrollbar bg-slate-950/50 rounded-xl border border-slate-800">
                            <table className="w-full text-left">
                                <thead className="bg-slate-800 text-slate-400 sticky top-0"><tr><th className="p-3">{t.player}</th><th className="p-3">{t.score}</th><th className="p-3">{t.mode}</th><th className="p-3">操作</th></tr></thead>
                                <tbody>
                                    {scores.length === 0 ? <tr><td colSpan="4" className="text-center p-8 text-slate-500">{t.noData}</td></tr> : scores.map(s => (
                                        <tr key={s.id} className="border-b border-slate-800 hover:bg-slate-800/50">
                                            ...
                                            <td className="p-3"><button onClick={() => deleteScore(s.id)} className="px-3 py-1 bg-red-900/50 hover:bg-red-600 text-red-200 rounded font-bold">刪除</button></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                ...
                {activeTab === 'snapshot' && (
                    <div className="flex-1 flex flex-col items-center justify-center p-4">
                        <div className="bg-slate-800 p-8 rounded-3xl border border-slate-700 max-w-lg w-full text-center shadow-2xl">
                            ...
                            <p className="text-slate-400 mb-8 text-sm leading-relaxed">將分散的紀錄壓縮成單一 JSON 檔案，徹底釋放空間與讀取次數。</p>
