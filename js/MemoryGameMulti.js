...
// 🧪 測試模式開關：要正式給學生玩時請改為 false，測試時請開 true
...
function MemoryGameMulti({ onBack, settings, wordDatabase, dbRef, user }) {
    ...
    useEffect(() => {
        ...
        const unsubscribe = dbRef.collection('rooms').doc(roomData.id).onSnapshot(doc => {
            if (doc.exists) {
                ...
                if (data.status === 'finished' && view !== 'result') setView('menu'); 
            } else {
                alert('房間已解散！');
                onBack();
            }
    ...
    const handleCreateRoom = async () => {
        ...
        if (!DEV_TEST_MODE && (!settings?.selectedUnits || settings.selectedUnits.length === 0)) {
            return setErrorMsg('請先回到主畫面勾選對戰的單字範圍！');
        }
    ...
    const handleJoinRoom = async () => {
        ...
        try {
            ...
            if (currentPlayersCount >= 4) return setErrorMsg('房間已滿 (上限 4 隊)');
    ...
    const handleStartGame = async () => {
        ...
        if (DEV_TEST_MODE) {
            // 🧪 4x5 測試模式：只需 6 組大寫配小寫字母 = 12 張單字牌
    ...
    if (view === 'menu') return (
        <div className="flex-1 flex flex-col items-center justify-center p-6 bg-slate-900 text-white animate-[fadeIn_0.4s_ease-out]">
            <div className="bg-slate-800 rounded-3xl shadow-2xl p-8 max-w-md w-full border border-slate-700 text-center">
                ...
                <h2 className="text-3xl font-black">{t.title} {DEV_TEST_MODE && <span className="text-xs bg-purple-600 text-white px-2 py-0.5 rounded-full vertical-middle ml-1">測試模式</span>}</h2>
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4 pt-2">
                        <button onClick={handleCreateRoom} className="p-4 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-black shadow-lg shadow-blue-600/20 transition-transform hover:scale-105">{t.createBtn}</button>
                    ...
                    <div className="space-y-2 mb-6 min-h-[140px]">
                        {playersList.map((p, i) => (
                            <div key={i} className="flex items-center justify-between bg-slate-700 p-3 rounded-xl border border-slate-600">
                                <span className="font-bold text-white">{p.name} {p.isHost && '👑'}</span>
                                <span className={`px-4 py-1 rounded-full font-black text-sm shadow-md ${teamColors[p.team]}`}>{p.team}</span>
                            </div>
                        ))}
                    </div>
                    {isHost ? (
                        <button onClick={handleStartGame} disabled={playersList.length < (DEV_TEST_MODE ? 1 : 2)} className="w-full py-4 bg-gradient-to-r from-blue-600 to-indigo-500 text-white rounded-xl font-black text-xl shadow-lg hover:scale-105 disabled:opacity-50 transition-all">
                            {playersList.length < (DEV_TEST_MODE ? 1 : 2) ? '等待對手加入...' : t.startBtn}
                        </button>
                    ) : (
    ...
    return (
        <div className="flex-1 flex flex-col p-3 sm:p-6 bg-slate-900 overflow-hidden text-white h-[100dvh]">
            <div className="block sm:hidden bg-indigo-900/80 border border-indigo-700 text-center py-1 text-xs font-bold rounded-lg mb-2">{t.rotateHint}</div>
            
            <header className="flex justify-between items-center mb-3 sm:mb-4 bg-slate-800 p-4 rounded-xl border border-slate-700 shrink-0 z-20">
                <button onClick={handleLeaveRoom} className="px-3 py-1.5 sm:px-4 sm:py-2 bg-slate-700 hover:bg-slate-600 rounded-lg text-white font-bold"><i className="fa-solid fa-arrow-left"></i></button>
                <div className="text-center flex-1">
                    <h2 className={`text-xl sm:text-3xl font-black ${isMyTurn() ? 'text-yellow-400 animate-pulse' : 'text-slate-300'}`}>{isMyTurn() ? t.myTurn : t.turn.replace('%team%', roomData.currentTeam)}</h2>
                    {isMyTurn() && roomData.turnState.comboCount > 0 && !roomData.activeEffect && <span className="text-xs sm:text-sm font-bold text-yellow-500 block">🌟 再翻一組機會！</span>}
                </div>
                <div className="text-right bg-slate-900 px-3 py-1.5 sm:px-4 sm:py-2 rounded-lg border border-slate-700">
                    <span className="text-[10px] sm:text-xs text-slate-500 font-bold block">分數</span>
                    <span className="text-base sm:text-2xl font-black text-emerald-400">{roomData.players[user.uid]?.score || 0}{localCoins > 0 && <span className="text-yellow-400 text-xs sm:text-base ml-1">+{localCoins}</span>}</span>
                </div>
            </header>

            <main className="flex-1 min-h-0 w-full flex items-center justify-center relative z-10 max-w-5xl mx-auto">
                {/* 🌟 修正為 grid-cols-5 的 4x5 完美佈局 */}
                <div className={`w-full h-full max-h-[80vh] grid grid-cols-5 gap-2 sm:gap-4 p-2 sm:p-4 rounded-3xl border transition-colors ${isTargetingBoard ? 'bg-purple-950/40 border-purple-500 shadow-[0_0_20px_rgba(168,85,247,0.4)]' : 'bg-slate-950 border-slate-800'}`}>
                    {roomData.board.map((card, index) => {
                        ...
                        const targetClass = isTargetable ? 'ring-4 ring-purple-400 animate-pulse cursor-crosshair' : (isTargetingBoard ? 'opacity-30' : '');
                        ...
                        return (
                            <button
                                ...
                                className={`relative w-full h-full rounded-xl sm:rounded-2xl flex flex-col items-center justify-center p-1 sm:p-2 aspect-[3/4] sm:aspect-auto transition-all duration-300 ${
                                    isFlipped || showRadarText
                                        ? 'bg-slate-800 border-2 border-slate-600 shadow-inner' 
                                        : 'bg-gradient-to-b from-blue-600 to-blue-800 hover:from-blue-500 shadow-xl border-2 border-blue-400/30'
                                } ${isPermanentlyFlipped ? 'opacity-10 pointer-events-none' : ''} ${targetClass}`}
                            >
                                {DEV_TEST_MODE && !isFlipped && !showRadarText && (
                                    <span className="absolute bottom-1 right-2 text-[10px] font-mono text-blue-400/30 font-bold">{card.matchId.substring(0,3)}</span>
                                )}

                                {!isFlipped && !isLocked && !showRadarText && <i className="fa-solid fa-globe text-blue-300/30 text-3xl sm:text-5xl"></i>}
                                {isLocked && !isFlipped && !showRadarText && <div className="text-center"><i className="fa-solid fa-lock text-slate-400 text-2xl sm:text-3xl"></i><span className="block text-[10px] sm:text-xs text-slate-500 mt-1">{card.lockedBy}</span></div>}
                                
                                {(isFlipped || showRadarText) && (
                                    card.isPowerUp ? (
                                        <div className={`text-center ${showRadarText ? 'opacity-30' : ''}`}><i className={`fa-solid ${card.icon} text-3xl sm:text-5xl text-yellow-400 mb-2`}></i><span className="block text-xs sm:text-base font-black text-slate-300">{card.text}</span></div>
                                    ) : (
                                        <span className={`font-black block break-all text-center leading-tight ${showRadarText ? 'opacity-30' : ''} ${card.type === 'en' ? 'text-cyan-400 font-mono text-lg sm:text-3xl' : 'text-slate-200 text-base sm:text-2xl'}`}>{card.text}</span>
                                    )
                                )}
                            </button>
                        );
                ...
                {isCoinRaining && (
                    <div className="absolute inset-0 pointer-events-none z-40 overflow-hidden rounded-2xl">
                        {coins.map(c => (
                            <div key={c.id} 
                                className={`absolute text-yellow-400 text-4xl sm:text-6xl animate-[fall_linear_forwards] ${roomData.activeEffect.triggerTeam === myTeam ? 'pointer-events-auto cursor-pointer hover:scale-125' : 'opacity-40'}`}
                            ...
                            >
                                <i className="fa-solid fa-coin drop-shadow-[0_0_15px_rgba(234,179,8,0.8)]"></i>
                            </div>
                        ))}
                    </div>
                )}
                ...
                {roomData.activeEffect && roomData.activeEffect.step === 'selecting' && (
                    <div className="absolute inset-0 bg-black/85 z-50 flex flex-col items-center justify-center rounded-3xl backdrop-blur-sm animate-[fadeIn_0.2s_ease-out]">
                        <h3 className="text-3xl sm:text-5xl font-black text-yellow-400 mb-4 text-center">{roomData.activeEffect.triggerTeam} 翻到了【{roomData.activeEffect.cardName}】</h3>
                        {roomData.activeEffect.triggerTeam === myTeam ? (
                            <div className="mt-4 flex flex-col items-center">
                                {roomData.activeEffect.type === 'auto' && (
                                    <button onClick={() => resolveEffect()} className="px-8 py-4 bg-yellow-500 hover:bg-yellow-400 text-slate-900 font-black rounded-full text-2xl shadow-[0_0_20px_rgba(234,179,8,0.5)] animate-bounce">立即發動效果！</button>
                                )}
                                {roomData.activeEffect.type === 'interactive_board' && (
                                    <p className="text-purple-400 font-bold text-xl sm:text-2xl animate-pulse bg-purple-950/80 px-8 py-4 rounded-full border border-purple-500"><i className="fa-solid fa-hand-pointer mr-2"></i>請點擊後方盤面上的任何一張隱藏卡...</p>
                                )}
                                {roomData.activeEffect.type === 'interactive_team' && (
                                    <div className="flex gap-4 mt-4">
                                        {roomData.turnOrder.filter(t => t !== myTeam).map(team => (
                                            <button key={team} onClick={() => resolveEffect(team)} className="px-8 py-4 bg-slate-800 hover:bg-slate-700 border-2 border-slate-500 rounded-2xl font-bold text-xl transition-transform hover:scale-105">使用在【{team}】</button>
                                        ))}
                                    </div>
                                )}
                            </div>
                        ) : (
                            <p className="text-slate-400 font-bold mt-6 text-2xl animate-pulse"><i className="fa-solid fa-hourglass-half mr-2"></i>等待對方執行策略動作...</p>
                        )}
                    </div>
                )}
            </main>
